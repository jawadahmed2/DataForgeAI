    ...
    def convert_pdf_to_markdown_and_save(self, pdf_file_paths):
        ...
        for pdf_file_path in pdf_file_paths:
            ...
            files.append(
                (
                    "pdf_files",
                    (os.path.basename(pdf_file_path), pdf_content, "application/pdf"),
                )
            )
        ...
        if response.status_code == 200:
            ...
            output_folder = os.path.splitext(os.path.basename(pdf_file_paths[0]))[0] # type: ignore
...
class AsyncDataForgeParse:
    ...
    async def __request__(
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                url, files=files, json=json, headers=headers, timeout=self.timeout
            )
    ...
    async def parse_document(
        ...
        response = await self.__request__(
            self.parse_document_endpoint, files={"file": file_data}
        )
        data = ParsedDocument(
            **response, source_path=file_path, output_folder=output_folder
        )
        if output_folder:
    ...
    async def parse_pdf(
        ...
        if file_ext != ".pdf":
            raise ValueError(
                f"The file must be a PDF (.pdf), but received a file of type {file_ext}"
            )

        async with aiofiles.open(file_path, "rb") as file:
            file_data = await file.read()
        response = await self.__request__(
            f"{self.parse_document_endpoint}/pdf", files={"file": file_data}
        )
        data = ParsedDocument(
            **response, source_path=file_path, output_folder=output_folder
        )
    ...
    async def parse_ppt(
        ...
        if file_ext not in [".ppt", ".pptx"]:
            raise ValueError(
                f"The file must be a PPT file (.ppt or .pptx), but received a file of type {file_ext}"
            )

        async with aiofiles.open(file_path, "rb") as file:
            file_data = await file.read()
        response = await self.__request__(
            f"{self.parse_document_endpoint}/ppt", files={"file": file_data}
        )
        data = ParsedDocument(
            **response, source_path=file_path, output_folder=output_folder
        )
    ...
    async def parse_docs(
        ...
        async with aiofiles.open(file_path, "rb") as file:
            file_data = await file.read()
        response = await self.__request__(
            f"{self.parse_document_endpoint}/docs", files={"file": file_data}
        )
        data = ParsedDocument(
            **response, source_path=file_path, output_folder=output_folder
        )
    ...
    async def parse_image(self, file_path: str) -> dict:
        ...
        return await self.__request__(
            f"{self.parse_media_endpoint}/image", files={"file": file_data}
        )
    ...
    async def parse_video(self, file_path: str) -> dict:
        ...
        async with aiofiles.open(file_path, "rb") as file:
            file_data = await file.read()
        return await self.__request__(
            f"{self.parse_media_endpoint}/video", files={"file": file_data}
        )
    ...
    async def parse_audio(self, file_path: str) -> dict:
        ...
        async with aiofiles.open(file_path, "rb") as file:
            file_data = await file.read()
        return await self.__request__(
            f"{self.parse_media_endpoint}/audio", files={"file": file_data}
        )
    ...
    async def process_image(
        ...
        if task not in self.image_process_tasks:
            raise ValueError(
                f"Invalid task. Choose from: {', '.join(self.image_process_tasks)}"
            )
        file_ext = os.path.splitext(file_path)[1].lower()
        if file_ext not in self.allowed_image_extentions:
            raise ValueError(

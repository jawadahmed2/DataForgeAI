...
class ImageObj(BaseModel):
    ...
    def set_mime_type(cls, values):
        ...
        if not mime_type and name:
            ...
            values["mime_type"] = mime_type # type: ignore
        return values
...
class ParsedDocument(BaseModel):
    ...
    def set_mime_type(cls, values):
        ...
        if has_tables:
            values["tables"] = [ # type: ignore
                table.model_dump() for table in markdown_to_tables(markdown_text)
            ]
        if isinstance(images, dict):
            values["images"] = [] # type: ignore
            for name, data in images.items():
                values["images"].append(ImageObj(name=name, bytes=data).model_dump()) # type: ignore
...
def markdown_to_tables(markdown: str) -> List[TableObj] | None:
    ...
    if markdown_tables:
        for i, table_md in enumerate(markdown_tables):
            ...
            data_rows = [
                row for row in rows[2:] if not set(row.strip(" |")).issubset(set(":-"))
            ]
            data = [
...
def save_images_and_markdown(response_data, output_folder):
    ...
    for pdf in response_data:
        ...
        pdf_output_folder = os.path.join(
            output_folder, os.path.splitext(pdf_filename)[0]
        )
        ...
        with open(
            os.path.join(pdf_output_folder, "output.md"), "w", encoding="utf-8"
        ) as f:
